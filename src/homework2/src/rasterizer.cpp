...
static double scalarCross2D(const Vector2f& a, const Vector2f& b)
{
...
{   
    ...
    // 初版实现简单的算法，使用叉积来判断某个点是否在三角形中
    double c1 = scalarCross2D(B - A, P - A);
    double c2 = scalarCross2D(C - B, P - B);
    double c3 = scalarCross2D(A - C, P - C);
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // 1. 2D的三角形，直接认为z等于0就可以了吗？然后我直接取出来前两个分量
    Vector2f A(v[0].x(), v[0].y());
    Vector2f B(v[1].x(), v[1].y());
    Vector2f C(v[2].x(), v[2].y());
    int minX = std::floor(A.x() < B.x() ? A.x() < C.x() ? A.x() : C.x() : B.x() < C.x() ? B.x() : C.x());
    int maxX = std::ceil(A.x() > B.x() ? A.x() > C.x() ? A.x() : C.x() : B.x() > C.x() ? B.x() : C.x());
    int minY = std::floor(A.y() < B.y() ? A.y() < C.y() ? A.y() : C.y() : B.y() < C.y() ? B.y() : C.y());
    int maxY = std::ceil(A.y() > B.y() ? A.y() > C.y() ? A.y() : C.y() : B.y() > C.y() ? B.y() : C.y());
